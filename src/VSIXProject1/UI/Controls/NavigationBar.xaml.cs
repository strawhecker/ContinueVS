    ...
    public partial class NavigationBar : UserControl, INotifyPropertyChanged
    {
        ...
        public int ToolCount
        {
            ...
            set
            {
                ...
                {
                    _toolCount = value;
                    LoggerService.Current.WriteDebug($"[g7-nav-b1] ToolCount changed: {value}");
        ...
        public NavigationBar()
        {
            ...
            {
                LoggerService.Current.WriteError($"[g7-nav-b1-err] InitializeComponent failed: {ex.Message}", ex);
                // Continue without XAML initialization; this can happen if the XAML file isn't in the project
            }
            LoggerService.Current.WriteDebug("[g7-nav-b1] NavigationBar() parameterless constructor called");
        ...
        {
            LoggerService.Current.WriteDebug("[g7-nav-b1-loaded] NavigationBar Loaded event fired");
            try
            {
                var sp = ViewModelLocator.ServiceProvider;
                if (sp == null)
                {
                    LoggerService.Current.WriteDebug("[g7-nav-b1-loaded] ViewModelLocator.ServiceProvider is null, ToolCount stays 0");
                    return;
                }
                ...
                if (configSvc == null)
                {
                    LoggerService.Current.WriteDebug("[g7-nav-b1-loaded] IConfigService not found in DI, ToolCount stays 0");
                    return;
                }
                ...
                LoggerService.Current.WriteDebug($"[g7-nav-b1-loaded] ToolCount initialized to {ToolCount}");
            }
            catch (Exception ex)
            {
                LoggerService.Current.WriteError($"[g7-nav-b1-loaded-err] Error: {ex.Message}", ex);
            }
        }
        ...
        public NavigationBar(IConfigService configService) : this()
        {
            ...
            LoggerService.Current.WriteDebug("[g7-nav-b2] NavigationBar(IConfigService) constructor called");
            RefreshToolCount();
            _configService.ConfigChanged += OnConfigChanged;
        }
        ...
        {
            LoggerService.Current.WriteDebug("[g7-nav-b3] ConfigChanged event received, refreshing tool count");
        ...
        private void RefreshToolCount(IConfigService? svc = null)
        {
            try
            {
                ...
                LoggerService.Current.WriteDebug($"[g7-nav-b4] RefreshToolCount: {count} tools enabled");
                ToolCount = count;
            }
            catch (Exception ex)
            {
                LoggerService.Current.WriteError($"[g7-nav-b5] RefreshToolCount error: {ex.Message}", ex);
                ToolCount = 0;
            }
        }
    }
