    ...
    public partial class ContinueToolWindowControl : UserControl, IDisposable, IGuiReplyProvider
    {
        ...
        public ContinueToolWindowControl()
        {
            ...
            try
            {
                ...
                // t4.4 - ServiceBootstrapper initialization and MessageDispatcher setup
                System.Diagnostics.Debug.WriteLine("[CV-t4.4] Initializing ServiceBootstrapper and MessageDispatcher (t5 begins)...");
                IDisposable? scope44 = tracer?.BeginScope("t4.4", "ContinueToolWindowControl.ServiceBootstrapper");
                try
                {
                    // Initialize DI container via ServiceBootstrapper
                    _serviceProvider = ServiceBootstrapper.ConfigureServices();
                    System.Diagnostics.Debug.WriteLine("[CV-t4.4] ✓ ServiceBootstrapper initialized");

                    // Create MessageDispatcher with service provider for factory-based handler resolution
                    _dispatcher = new MessageDispatcher(
                        serviceProvider: _serviceProvider);
                    System.Diagnostics.Debug.WriteLine("[CV-t4.4] ✓ MessageDispatcher created with IServiceProvider");
                }
                finally
                {
                    scope44?.Dispose();
                ...
                // t4.5 - Handler registration (t5 continues)
                System.Diagnostics.Debug.WriteLine("[CV-t4.5] Registering message handlers...");
                IDisposable? scope45a = tracer?.BeginScope("t4.5", "ContinueToolWindowControl.HandlerRegistration");
                try
                {
                    RegisterHandlers();
                    System.Diagnostics.Debug.WriteLine("[CV-t4.5] ✓ All handlers registered");
                }
                finally
                {
                    scope45a?.Dispose();
                ...
                System.Diagnostics.Debug.WriteLine($"[b20-HANDLER-COUNT] Registered handlers: {_dispatcher?.GetHandlerCount() ?? 0}");
                ...
                // t4.6 - Loaded event wiring (prelude to t22+)
                System.Diagnostics.Debug.WriteLine("[CV-t4.6] Wiring Loaded event...");
                IDisposable? scope46 = tracer?.BeginScope("t4.6", "ContinueToolWindowControl.LoadedEvent");
                try
                {
                    Loaded += OnLoaded;
                    System.Diagnostics.Debug.WriteLine("[CV-t4.6] ✓ Loaded event wired");

                    // Wire Unloaded event for bridge teardown (b15)
                    Unloaded += OnUnloaded;
                    System.Diagnostics.Debug.WriteLine("[CV-t4.6] ✓ Unloaded event wired");
                }
                finally
                {
                    scope46?.Dispose();
...
#pragma warning restore VSSDK007
        ...
        private void RegisterHandlers()
        {
            ...
            // IDE Info handlers (stateless, direct registration)
            ...
            // History handlers - GUI requests session history on init
...
#pragma warning restore VSSDK007
        ...
        private System.Threading.Tasks.Task OnWebMessageReceivedAsync(object sender, CoreWebView2WebMessageReceivedEventArgs e)
        {
            ...
            if (_dispatcher == null)
            {
                System.Diagnostics.Debug.WriteLine($"[DISPATCH-ERROR] MessageDispatcher not initialized for message type: {message.MessageType}");
                SendErrorReplyToGui(message.MessageType, message.MessageId, "Dispatcher not initialized");
                return System.Threading.Tasks.Task.CompletedTask;
            }
