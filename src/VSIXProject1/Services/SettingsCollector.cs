...
namespace ContinueVS.Services
{
    ...
    internal static class SettingsCollector
    {
        ...
        public static async Task<Dictionary<string, object>> ReadSettingsAsync()
        {
            ...
            LoggerService.Current.WriteDebug($"[b16-CONFIG-READ] ReadSettingsAsync entry");
            ...
            lock (s_cacheLock)
            {
                if (s_cachedSettings != null && !s_cachedSettings.IsExpired)
                {
                    ...
                    LoggerService.Current.WriteDebug($"[b16-CONFIG-READ] Cache hit, elapsed={configReadStopwatch.ElapsedMilliseconds}ms");
            ...
            configReadStopwatch.Stop();
            LoggerService.Current.WriteDebug($"[b16-CONFIG-READ] File read completed, elapsed={configReadStopwatch.ElapsedMilliseconds}ms, settingCount={settings?.Count ?? 0}");
        ...
        public static void ClearCache()
        {
            ...
            LoggerService.Current.WriteDebug("[b19-CACHE-INVALIDATE] Settings cache cleared");
            lock (s_cacheLock)
            {
