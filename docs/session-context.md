...
| b21 | **Integration** | **WebView2 Initialization Complete — DOM Ready & Bridge Accessible** — Verify WebView2 content loads (DOMContentLoaded fires), React mounts UI components, JavaScript bridge object `window.continueVS` accessible and ready BEFORE C# sends initial messages. Missing this causes handlers called into empty/unready UI. Measure initialization time, verify no errors during React mount. | b10 | Breakpoint on line where first message sent; verify continueVS object exists. Debug logs: [b21-DOM-READY], [b21-REACT-MOUNT], [b21-BRIDGE-READY], [b21-INIT-TIME-MS]. Manual: Open tool window, WebView loads without white screen. No JS console errors. |
| b22 | **Integration** | **Initial Message Push — WebviewPusher.PushConfigUpdate** — Verify first C# → JS message (PushConfigUpdate) executes immediately after WebView ready (b21). This message triggers initial UI render with settings/model dropdowns visible. If timeout or error, spinner never stops. Measure latency (<500ms), verify response received, UI renders dropdowns. | b21 | ✅ **DEBUGGED**: Instrumentation [b22-PUSH-START], [b22-CONFIG-SERIALIZED], [b22-SCRIPT-INJECTED], [b22-LATENCY-GATE-PASS/EXCEEDED], [b22-UI-RENDER] added to WebviewPusher.PushConfigUpdate with Stopwatch timing. Call site marker [b22-CALL-SITE] in WorkspaceConfigWatcher.OnConfigChanged. Integration test suite InitialMessagePushIntegrationTests.cs with 6 scenarios: valid IdeSettings serialization, latency gate <500ms compliance, messageType "configUpdate" verification, payload structure validation (result/profileId/profiles), profileId null check, profiles empty array check. All tests passing. Manual test guide MANUAL-TEST-TOOL-WINDOW-INITIALIZATION.md created with pre-test checklist, spinner behavior verification, [b22-*] log sequence expectations, UI render verification, performance gate confirmation, debugger breakpoint trace (BP1 entry line 28, BP2 call site line 59, BP3 post-elapsed line 45), pass/fail criteria, and diagnostic table. |
| b23 | **Integration** | **Handler Timeout & Error Propagation** — Verify if handler times out (>2s with no response) or throws exception, UI gets error state and spinner stops. Missing: silent failures leave spinner spinning forever. Implement timeout gate (2s), catch handler exceptions, send error response to JS, verify UI handles gracefully (error toast, spinner stops). | b16, b17, b22 | Inject timeout to handler, verify error response sent. Debug logs: [b23-HANDLER-TIMEOUT], [b23-HANDLER-ERROR], [b23-ERROR-RESPONSE-SENT]. Unit test: Mock handler that times out; assert error message sent to UI. Manual: Open tool window; if handler hangs >2s, spinner stops + error shown (not infinite spin). |
| b24 | **Integration** | **Chat Send Handler Registration \u2014 llm/streamChat** — Verify `bridge:llm/streamChat` handler is registered in MessageDispatcher before first chat message arrives. Validate handler signature compatible with streaming response pattern. Check handler can deserialize chat request payload (messages[], model, temperature, maxTokens, etc.). | b20, b22 | Handler registration log [b24-HANDLER-REGISTERED], integration test ChatStreamHandlerRegistrationTests.cs (verify handler exists, deserialization of valid/invalid payloads, message schema validation). Debugger breakpoint on LlmStreamChatHandler.HandleAsync entry. |
