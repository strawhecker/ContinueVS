...
|---|---|---|---|---|
| b4 | **Integration** | **Bridge Global Object Injection (C# → JavaScript)** — Verify `window.continueVS` object is defined after injection, confirm `window.continueVS.sendMessage` and `window.continueVS.onMessage` callable. **Scope**: Structural only (object exists, signatures available via `typeof`). Functional testing deferred to b11–b15. | None | Logs `[B4-*]` + breakpoints at injection site |
| b5 | **Unit** | **Bridge Message Envelope Structure Validation** — Verify C# Message class JSON serialization/deserialization. Round-trip fidelity, null/empty field handling. No WebView required; mocked dependencies. | None | Unit test assertions (no logs needed) |
| b6 | **Unit** | **MessageDispatcher Handler Registration** — Verify Register(), lookup by type, idempotency. All 19+ handlers register without conflict. Case-insensitive lookup (if applicable). No execution; mocked handlers only. | None | Unit test assertions |
| b7 | **Unit** | **MessageDispatcher Dispatch Routing (Mocked)** — Inject mock message (MessageType: "test-handler", MessageId: "msg-001"). Verify dispatcher finds handler, `handler.HandleAsync()` invoked. Callback/event confirms invocation. No WebView, mocks only. | b6 | Unit test assertions |
| b8 | **Unit** | **Message Validator — Valid Envelope** — Construct valid Message (all required fields), pass to MessageValidator. Assert validation succeeds without exception. | b5 | Unit test assertions |
| b9 | **Unit** | **Message Validator — Invalid Envelope (Missing Fields)** — Construct Message missing `MessageType` or `MessageId`. Pass to MessageValidator. Assert validation fails (exception or error). | b5 | Unit test assertions |
| b10 | **Unit** | **IMessageHandler Contract — Mock Execution** — Create mock handler implementing `IMessageHandler`. Verify `HandleAsync(Message, CancellationToken)` executes with correct parameters. Validate CancellationToken is honored. No real logic. | None | Unit test assertions |
