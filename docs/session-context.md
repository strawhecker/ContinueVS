...
| b16 | **Integration** | **Bridge Handler Response: loadSettings (Settings Configuration)** — Verify `bridge:loadSettings` handler is registered and returns complete, well-formed settings response (model, provider, temperature, contextWindow, maxTokens). Validate JSON validity and escaping. Ensure no timeout/deadlock (p99 < 100ms). Spinner must disappear when response received. | None | Unit test: Mock SettingsCollector, invoke handler, assert response shape. Integration test: Real WebView, send `bridge:loadSettings`, capture ExecuteScriptAsync result, validate JSON. Debug logs: [b16-REQUEST-RECEIVED], [b16-CONFIG-READ], [b16-RESPONSE-SERIALIZED], [b16-SCRIPT-INJECTED]. |
| b17 | **Integration** | **Bridge Handler Response: getModelInfo (Model Selection Dropdown)** — Verify `bridge:getModelInfo` handler is registered and returns complete model metadata without spinner indefinitely. Validate C# `ModelInfoCollector` doesn't hang/deadlock. Return currentModel + availableModels array with all configured models. Response latency < 50ms. | None | Unit test: Mock ModelInfoCollector, invoke handler, assert ≥1 model in response. Integration test: Send `bridge:getModelInfo`, capture result, validate structure. Debug logs: [b17-REQUEST-RECEIVED], [b17-COLLECTOR-QUERY], [b17-MODEL-MAPPING], [b17-RESPONSE-SERIALIZED], [b17-SCRIPT-INJECTED]. |
...
| b19 | **Integration** | **Model Dropdown Handler Round-Trip (Query → Display → Select)** — End-to-end: UI queries model info via b17, displays dropdown without spinner, user selects model, change persists. Handler updates current model in config. Subsequent `bridge:getModelInfo` calls confirm new selection without race conditions. | b16, b17 | Manual test: Open model dropdown, measure spinner duration, select model, verify persistence. Integration test: Rapid model info queries, assert consistent responses. Integration test: Model selection change persists across re-queries. Debug logs: [b19-*] selection flow with request/response timestamps. |
