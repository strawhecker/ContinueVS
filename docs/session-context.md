...
| c15-rev | 2: Cache Population | **Canary Test** | **Seed Cache with Mock Tools (Static Test Data)** — Add static mock data to `ConfigCache.cs` constructor with 3 mock tools (read_file, search_codebase, edit_existing_file). Add logging: `System.Diagnostics.Debug.WriteLine("[c15-CACHE-SEEDED] Cache initialized with 3 mock tools");`. This tests cache plumbing without real data. | c14 | Debug output shows `[c15-CACHE-SEEDED]`. GUI loads, and tool dropdown shows 3 items. |
