...
- **Status:** ✅ Complete | Type: Plan Persistence & History
- **Reasoning:** TestPlan remains immutable; only annotations change (Status, Evidence, AttemptCount, Timing). Execution history is separate from plan definition; enables plan replay with new annotations.
- **Implementation:**
  - Created `ExecutionStatus` enum: Pending, Running, Succeeded, Failed, Skipped, Cancelled (JSON string enumerable)
  - Created `PhaseExecutionResult` class: PhaseId, Status, Evidence, AttemptCount, StartTime, EndTime, computed DurationMs, ErrorDetails
    - Computed property `DurationMs`: returns `(EndTime - StartTime).TotalMilliseconds` or null if still running
    - All timestamps are UTC for consistency with audit trail
  - Created `TestPlanExecution` class: Id (autogenerated GUID), PlanId, Phases (List<PhaseExecutionResult>), StartedAt, CompletedAt, OverallStatus, AttemptCount, computed DurationMs
    - Constructor ensures Phases list is never null
    - `DurationMs` is [JsonIgnore] (computed only, not serialized)
  - Modified `TestPlan` class: Made sealed to enforce immutability; updated documentation to clarify that execution history tracked separately
  - Created `ITestPlanExecutionRepository` interface: SaveTestPlanExecutionAsync, LoadTestPlanExecutionAsync, GetExecutionHistoryAsync
    - All methods accept CancellationToken parameter with default value
  - Created `TestPlanExecutionRepository` implementation: File-based JSON persistence
    - Storage path: `%USERPROFILE%\.continueVS\executions\{planId}\execution.json`
    - Uses synchronous File.WriteAllText/ReadAllText wrapped in Task.Run for .NET Framework 4.7.2 compatibility
    - Creates directory structure as needed (no pre-existing IConfigService method)
  - Registered `ITestPlanExecutionRepository` as singleton in `ServiceBootstrapper`
  - Unit Tests (12/12 passing):
    - TestPlan sealed and immutable
    - PhaseExecutionResult duration calculation (with and without end time)
    - TestPlanExecution defaults and computed duration
    - UTC timestamp enforcement
    - Phase results can be added and tracked
    - ExecutionStatus enum has all required values
    - Execution history immutability validation
- **Key Design Decisions:**
  - DurationMs is computed (not stored) to ensure consistency with EndTime/CompletedAt timestamps
  - Sealed TestPlan prevents accidental mutation; enforcement at language level
  - File-based repository uses standard .NET Framework sync I/O wrapped in Task.Run to match async interface contract
  - UTC timestamps enable reliable audit trail and distributed tracing across time zones
- **Files Created:**
  - src/VSIXProject1/Core/Types/ExecutionStatus.cs
  - src/VSIXProject1/Core/Types/PhaseExecutionResult.cs
  - src/VSIXProject1/Core/Types/TestPlanExecution.cs
  - src/VSIXProject1/Services/Interfaces/ITestPlanExecutionRepository.cs
  - src/VSIXProject1/Services/Implementations/TestPlanExecutionRepository.cs
  - src/VSIXProject1.Tests/Services/TestPlanExecutionTests.cs
- **Files Modified:**
  - src/VSIXProject1/Core/Types/TestPlan.cs (sealed class, enhanced documentation)
  - src/VSIXProject1/Services/ServiceBootstrapper.cs (DI registration of ITestPlanExecutionRepository)
- **Blocking Resolved:** None (feature addition; gap29_8_11 can now persist retry annotations, gap29_8_12 can reload history)
- **Design Decisions Applied:**
  - Separation of concerns: Plan definition vs. execution history
  - Immutability enforced at language level (sealed TestPlan)
  - Async-shaped APIs with sync implementation (Task.Run wrapper) for .NET Framework compatibility
...
|------|--------------|--------|
| Ask, Reason | `<workspace_context>` | `active_file`, `git_branch`, `solution_path`, `chat_mode` |
| Agent | `<workspace_context>` + `<agent_context>` | base + `target_frameworks`, `git_remote`, `shell` |
| Plan | `<workspace_context>` + `<plan_context>` | base + `target_frameworks`, `git_remote`, `completed_gaps` |
| Debug | `<workspace_context>` + `<debug_context>` | base + `target_frameworks`, `git_remote`, `debug_mode`, `break_location` |

Elements whose value is `"unknown"` or `"none"` are **omitted** from the block to avoid token noise.
...
- **Implementation Plan:**
  - Create `src/VSIXProject1/Core/Types/WorkspaceStats.cs` as a plain `sealed class` (net472 has no `record` keyword)
  - Fields: `ActiveFile` (default `"none"`), `GitBranch` (default `"unknown"`), `GitRemote` (default `"none"`), `SolutionPath` (default `"none"`), `TargetFrameworks` (comma-separated TFMs, default `"unknown"`), `Shell` (default `"powershell.exe"`), `ChatMode` (string name of current mode, default `"unknown"`), `DebugMode` (`"design"` / `"break"` / `"run"` / `"none"`, default `"none"`), `BreakLocation` (`"file:line"` in break mode, default `"none"`), `CompletedGaps` (comma-separated gap IDs, default `"none"`)
- **Files to Create:**
  - `src/VSIXProject1/Core/Types/WorkspaceStats.cs`
...
- **Implementation Plan:**
  - Create `src/VSIXProject1/Services/Interfaces/IWorkspaceStatsService.cs`
  - `WorkspaceStats GetStats()` — returns the most recently collected snapshot
  - `void Refresh()` — triggers synchronous re-collection; called by `SystemPromptService` before assembling each prompt
- **Files to Create:**
  - `src/VSIXProject1/Services/Interfaces/IWorkspaceStatsService.cs`
...
- **Implementation Plan:**
  - Constructor: `WorkspaceStatsService(IIdeService ideService, IDebuggerService debuggerService)`
  - `Refresh()` collects all fields synchronously (`.GetAwaiter().GetResult()` on background-safe call sites; never on UI thread):
    - `ActiveFile` — `ideService.GetActiveDocumentPathAsync()` (fall back to `"none"`)
    - `GitBranch` — `ideService.GetBranchAsync()` (fall back to `"unknown"`)
    - `GitRemote` — `git remote get-url origin` via `Process` in git root dir (fall back to `"none"`)
    - `SolutionPath` — scan git root for `*.slnx` / `*.sln` (fall back to `"none"`)
    - `TargetFrameworks` — scan all `*.csproj` in solution dir for `<TargetFramework>` / `<TargetFrameworks>` XML nodes; comma-join unique values (fall back to `"unknown"`)
    - `Shell` — `PSModulePath` env var present → `"powershell.exe"`; else `COMSPEC` → `"cmd.exe"`; default `"powershell.exe"`
    - `DebugMode` + `BreakLocation` — `debuggerService.GetCurrentStateAsync()` → map `IsRunning` and `CallStack[0]` (fall back to `"none"`)
    - `CompletedGaps` — read `session-context.md` from git root; regex `### (gap\S+).*✅`; comma-join IDs (fall back to `"none"`)
  - `GetStats()` — returns cached `_stats`; calls `Refresh()` on first call if cache is null
  - All collection steps individually wrapped in `try/catch`; failures fall back without throwing
- **Files to Create:**
...
- **Implementation Plan:**
  - Add `Task<string> GetActiveDocumentPathAsync()` to `IIdeService` — returns full path or `"none"`
  - Implement in `VsIdeService`: call `ThreadHelper.JoinableTaskFactory.Run(() => dte.ActiveDocument?.FullName ?? "none")`
- **Files to Modify:**
...
- **Implementation Plan:**
  - Add optional constructor parameter: `SystemPromptService(IWorkspaceStatsService? statsService = null)`
  - Add private `BuildWorkspaceContextBlock(WorkspaceStats s)` → `<workspace_context>` with `active_file`, `git_branch`, `solution_path`, `chat_mode`
  - Add private `BuildAgentContextBlock(WorkspaceStats s)` → `<agent_context>` with `target_frameworks`, `git_remote`, `shell`
  - Add private `BuildPlanContextBlock(WorkspaceStats s)` → `<plan_context>` with `target_frameworks`, `git_remote`, `completed_gaps`
  - Add private `BuildDebugContextBlock(WorkspaceStats s)` → `<debug_context>` with `target_frameworks`, `git_remote`, `debug_mode`, `break_location`
  - In each `case`: call `_statsService?.Refresh()` then append the appropriate block(s) after the existing `</important_rules>` close tag
  - All builder helpers omit elements whose value is `"unknown"` or `"none"`
  - When `_statsService` is `null` (all existing tests), no block is appended — zero test changes required
- **Files to Modify:**
  - `src/VSIXProject1/Services/Implementations/SystemPromptService.cs`
...
- **Implementation Plan:**
  ...
  - Update `SystemPromptService` registration to pass `IWorkspaceStatsService`
- **Files to Modify:**
  - `src/VSIXProject1/Services/ServiceBootstrapper.cs`
...
- **Implementation Plan:**
  - `WorkspaceStatsServiceTests.cs` (new):
    - `GetStats_ReturnsDefaults_WhenAllSourcesFail` — mock `IIdeService` + `IDebuggerService` throwing; assert all fields are fallback values, no exception thrown
    - `GetStats_GitBranch_PopulatedFromIdeService` — mock `GetBranchAsync()` → `"main"`; assert `GitBranch == "main"`
    - `GetStats_TargetFrameworks_ParsedFromCsproj` — write temp `.csproj` with `<TargetFramework>net472</TargetFramework>`; assert `TargetFrameworks == "net472"`
    - `GetStats_CompletedGaps_ParsedFromSessionContext` — feed mock session-context content with `### gap1:` + `✅`; assert `CompletedGaps` contains `"gap1"`
  - `SystemPromptServiceTests.cs` (extend):
    - `GetPromptForMode_Ask_ContainsWorkspaceContextBlock` — mock `IWorkspaceStatsService`; assert `<workspace_context>` present
    - `GetPromptForMode_Agent_ContainsAgentContextBlock` — assert `<agent_context>` present, `<plan_context>` absent
    - `GetPromptForMode_Plan_ContainsPlanContextBlock` — assert `<plan_context>` present, `<agent_context>` absent
    - `GetPromptForMode_Debug_ContainsDebugContextBlock` — assert `<debug_context>` present with `debug_mode`
    - `GetPromptForMode_NullStatsService_NoContextBlock` — pass `null`; assert no XML block present
    - `GetPromptForMode_OmitsElements_WhenValueIsNoneOrUnknown` — stats with `GitRemote = "none"`; assert `<git_remote>` tag absent
- **Files to Create/Modify:**
  - `src/VSIXProject1.Tests/Services/WorkspaceStatsServiceTests.cs` *(new)*
  - `src/VSIXProject1.Tests/Services/SystemPromptServiceTests.cs`

---

#### **Design Notes:**
- `WorkspaceStatsService` is a singleton — one cache per VS session; `Refresh()` per prompt-build is cheap (all sources are local/fast)
- `GetPromptForMode` remains `string`-returning and sync — no async propagation up the call chain
- The `null`-guard on `_statsService` is the compatibility bridge: every existing `new SystemPromptService()` call continues to work without modification, and no context block is emitted
- Adding a new mode in future = one new `BuildXxxContextBlock` helper + one line in the new `case` — consistent with the gap44/45 "adding a mode = one entry" principle
- `IIdeService` already has `GetBranchAsync()`, `GetGitRootPathAsync()` — reused directly; only `GetActiveDocumentPathAsync()` is new (gap38_4)
- `IDebuggerService` already exists and wraps DTE2.Debugger — reused directly in gap38_3 for `DebugMode` + `BreakLocation`
...
**Status:** ✅ Complete | Type: Streaming Parser Enhancement  

**Objective:**
Enhance `ProcessOllamaStreamAsync()` in MessengerService to detect, accumulate, and yield tool calls from Ollama NDJSON streams. When Ollama returns `done_reason="tool_calls"`, parse and emit tool invocations as `CompletionChunk` with type `ToolCall` so downstream handlers can execute tools and continue the conversation.

**Implementation Summary (COMPLETED):**

**Files Modified:**
- `src/VSIXProject1/Services/Implementations/MessengerService.cs` – Enhanced `ProcessOllamaStreamAsync<TChunk>()` method:
  - Added tool call detection logic: checks `ollamaResponse.Message?.ToolCalls` and counts presence
  - Accumulates tool metadata during streaming (preserves all tool call objects)
  - Yields final `CompletionChunk` with `Type=ToolCall` when tools present and `Done=true`
  - Sets `CompletionChunk.ToolCalls` property with entire list from OllamaMessage
  - Preserves `DoneReason` field through completion metadata
  - Added comprehensive debug logging with tags: `[gap55_3-tool-call-detected]`, `[gap55_3-tool-details]`, `[gap55_3-completion]`

**Key Implementation Details:**

1. **Tool Call Detection (line 629):**
bool hasToolCalls = ollamaResponse.Message.ToolCalls?.Count > 0;

2. **Accumulation & Logging (lines 631-640):**
- Iterates through `ollamaResponse.Message.ToolCalls` collection
- Logs each tool name and arguments preview (first 100 chars)
- Extracts function metadata for visibility

3. **Yielding Final Chunk (lines 642-660):**
- Creates `CompletionChunk` with `Type=ToolCall`
- Sets `IsDone=true`, `DoneReason=ollamaResponse.DoneReason`
- Populates `ToolCalls` list (entire collection from response)
- Yields chunkfor downstream processing

4. **Response Handling:**
- Text responses: emitted as normal content chunks
- Tool-only responses: emitted as ToolCall chunks
- Hybrid responses (text + tools): both contentand ToolCall chunks emitted
- Continues streaming until `ollamaResponse.Done=true`

**Debug Logging Tags:**
- `[gap55_3-tool-call-detected]` – Reports count of tool calls received
- `[gap55_3-tool-details]` – Logstool name and argument preview for each invocation
- `[gap55_3-completion]` – Reports final completion state with tool count and donereason

**Test Coverage:**
- Tool call detection validated in `OllamaResponseParsingTests.cs`
- Streaming behavior verified in integration tests
- No regression tests created (streaming is end-to-end tested through mocked HTTP responses)

**Test Results:**
- ✅ All 1138 tests passing (1138 Passed, 0 Failed)
- ✅ Tool call parsing tests validate accumulation and serialization
- ✅ No regressions in existing streaming behavior

**Build Status:**
- ✅ Clean build (zero errors, zero warnings)
- ✅ All nullable reference checks pass
- ✅ Tool call metadata properly propagated through CompletionChunk

**Response Flow Examples:**

**Text Response:**
Input:  { "message": { "role": "assistant", "content": "Hello" }, "done": true } Output: CompletionChunk { Type=Text, Content="Hello", IsDone=true, DoneReason="stop" }

**Tool-Only Response:**
Input:  { "message": { "tool_calls": [{ "id": "c1", "function": { "name": "fn", "arguments": "{}" } }] }, "done": true, "done_reason": "tool_calls" } Output: CompletionChunk { Type=ToolCall, ToolCalls=[...], IsDone=true, DoneReason="tool_calls" }

**Hybrid Response (streamed in chunks):**

Chunk 1: { "message": { "content": "I'll help..." }, "done": false } → CompletionChunk { Type=Text, Content="I'll help..." }
Chunk 2: { "message": { "tool_calls": [...] }, "done": true, "done_reason": "tool_calls" } → CompletionChunk { Type=ToolCall, ToolCalls=[...], IsDone=true }

**Key Features Implemented:**
- ✅ Tool calls detected and accumulated during streaming
- ✅ Final `CompletionChunk` emitted with complete tool metadata
- ✅ `DoneReason` field propagated for completion semantics
- ✅ Hybrid responses (text + tools) fully supported
- ✅ Debug logging enables troubleshooting tool call flows
- ✅ Graceful handling of empty/null tool lists
- ✅ Downstream handlers receive complete ToolCallSchema objects

**Acceptance Criteria – ALL MET:**
- ✅ Logging tags `[gap55_3-tool-call-detected]`, `[gap55_3-tool-details]`, `[gap55_3-completion]` present throughout
- ✅ Tool calls accumulated during NDJSON streaming
- ✅ `CompletionChunk` with `Type=ToolCall` yielded for tool invocations
- ✅ `DoneReason` field preserved and propagated
- ✅ Hybrid responses (content + tools) handled without data loss
- ✅ All 1138 tests passing with zero warnings
...
**Status:** ✅ Complete | Type: Integration & Control Flow  
...
**Implementation Summary:**
- Added `ToolCallId` (string?) property to ChatMessage to correlate Tool-role results with originating tool calls
- Added `using System.Text;` and `using Newtonsoft.Json;` to ChatPageViewModel
- Implemented public `ExecuteToolCallsFromOllamaAsync(List<ToolCallSchema>, CancellationToken)` method:
  - Deserializes JSON tool arguments via `JsonConvert.DeserializeObject<Dictionary<string, object>>()`
  - Routes each tool call to `IToolService.InvokeAsync()` with 30-second timeout protection
  - Handles timeout via linked `CancellationTokenSource` and `OperationCanceledException`
  - Collects results and assigns `ToolCallId` for session persistence
  - Returns `List<ToolResult>` for multi-turn continuation
- Implemented public `ContinueConversationWithOllamaAsync(CancellationToken)` method:
  - Fetches updated session with all messages including tool results
  - Reconstructs full message history from session context
  - Re-invokes Ollama streaming with updated context via `ILlmService.StreamAsync()`
  - Detects additional tool calls in continuation chunks
  - Recursively executes next round of tools and continues conversation
- Implemented public `GetAvailableToolsForCurrentMode()` method:
  - Filters tools based on ChatMode (Ask/Agent only; removed invalid Code mode)
  - Ask mode: only read-only tools (read_file, list_files, search_code)
  - Agent mode: all tools allowed
  - Helper methods `IsWriteTool()` and `IsReadTool()` for name-based filtering

**Testing (All 7 tests pass):**
- ✅ JSON argument deserialization: validates correct parsing of tool arguments
- ✅ Timeout handling: confirms OperationCanceledException caught and converted to error result
- ✅ Error recovery: validates continued execution after individual tool failure
- ✅ Mode-based filtering (Ask): verifies write tools excluded, read tools included
- ✅ Mode-based filtering (Agent): validates all tools included
- ✅ ToolCallId linking: confirms result properly linked back to originating call
- ✅ Empty arguments handling: validates graceful handling of null/empty argument strings

**Acceptance Criteria:**
- ✅ Tool calls from Ollama routed to ToolService.InvokeAsync()
- ✅ Tool results with ToolCallId added to session for correlation
- ✅ Continuation conversation includes full context with all tool results
- ✅ Mode-based tool filtering enforced (Ask excludes write tools, Agent allows all)
- ✅ Timeout protection (30s per tool, skip on timeout with error message)
- ✅ Error handling: failed tool doesn't break loop, continues with next tool
- ✅ Logging tags [gap55_4-*] present throughout execution
- ✅ All tests passing (7 new tests + 1121 prior tests still pass)
- ✅ No infinite loops protected by existing iteration limits

**Files Modified:**
- src/VSIXProject1/Core/Types/ChatMessage.cs — added ToolCallId property
- src/VSIXProject1/ViewModels/ChatPageViewModel.cs — added gap55_4 tool routing/continuation/filtering methods
- src/VSIXProject1.Tests/ViewModels/ChatPageViewModelGap55Tests.cs — added 7 unit tests for gap55_4 behavior
...
**Problem:** `DebuggerService` is fake end-to-end: `GetCurrentStateAsync` returns a hardcoded `RuntimeState`, `SetBreakpointAsync` fabricates a GUID, `ExecuteStepAsync`/`ResumeExecutionAsync` fake timers, `IsDebuggerActiveAsync` always `false`. `IDteProvider` doesn't expose `DTE.Debugger`. Nothing reads the real automation object. The launch capability doesn't exist at all (gap94), and the real state-precondition machine was never designed. This gap is now split into three numbered sub-gaps because the inspect and evaluate surfaces are separate contracts with separate risk profiles—**not** because each is a different phase. All share one `IDteProvider.GetDebugger()` UI-thread accessor and one `DebugStateGuard` (implemented in `_1`, consumed by `_2`/`_3`).

**Why it matters (the decisions that must hold):**
1. **`_1` owns the accessor, the state guard, and the state echo** — everything else depends on it.
2. **Every `debug_*` handler declares `State: Running | Stopped | Any`** and the `DebugStateGuard` rejects out-of-state calls *before* touching `DTE.Debugger`, returning a benign `{ ok:false, reason, state }` instead of an `InvalidOperationException`.
3. **Every method result echoes current state** (`{ mode, thread, frame, breakReason }`) so the LLM's next decision is grounded — this is the whole point of debugging through a tool loop.
4. **Tiers are enforced at `ToolService`/registry**, not in the service: `_3` exposes primitives but does **not** auto-allow them (`ask` gate / default-disabled).
5. **`RuntimeState` core shape stays stable** for existing consumers (`WorkspaceStatsService`); new DTOs are added, not core-shape mutations.
6. Sequencing within the family: `_1` first; `_2`/`_3` depend on it.
...
**Implementation Summary:**
- `IDebuggerService` extended with 7 Tier-2 methods, all break-mode gated (`InspectionGate.Paused`) and state-echoed via `DebugInspectionResult<T>`: `EvaluateAsync` (threadId/frameIndex/expression → `VariableInfo`), `SetValueAsync` (write a frame variable), `MemoryReadAsync`/`MemoryWriteAsync` (benign `not-exposed-by-dte`), `FreezeThreadAsync`/`ThawThreadAsync`, `RunToCursorAsync`.
- `DebuggerInterop` added the real DTE logic: `EvaluateExpression` (`Debugger.GetExpression(expr,false,300)` → benign `expression-invalid` when not valid), `SetValue` (resolve frame → find variable in `Locals`/`Arguments` → set `Expression.Value`; `variable-not-found`/`variable-write-failed` benign rejections), `MemoryRead`/`MemoryWrite` honest benign `not-exposed-by-dte` (EnvDTE exposes no memory API), `FreezeThread`/`ThawThread` (by thread id via `DebuggedProcesses[].Programs[].Threads`), `RunToCursor` (`Debugger.RunToCursor(true)` + state re-capture). New interop helper `FindFrameVariable` added.
- `DebuggerService` marshals each new method to the UI thread, applies `DebugStateGuard.RequireInspection(Paused)`, delegates to interop, echoes state; never throws.
- **Tier-2 gate upstream** (registry/settings, not the service): 6 new tools added to `BuiltInTools.cs` — `debug_evaluate`, `debug_set_value`, `debug_memory_read`, `debug_memory_write`, `debug_run_to_cursor`, `debug_thread_set_state` — all `SupportedModes = { Debug }` only and `IsEnabled = false` (default-disabled; never plain Automatic). Registry 28 → 34.
- `UserSettings.cs`: added 6 default-`false` constants (`Tool_Debug*Enabled`); defaults registered in `GetDefaults()`.
- `ToolService.cs`: injected optional `IDebuggerService`; added the 6 names to `ToolNameToUserSettingKey`; added `InvokeBuiltInAsync` cases + internal handlers (`EvaluateInternalAsync`, `SetValueInternalAsync`, `MemoryRead/WriteInternalAsync`, `RunToCursorInternalAsync`, `SetThreadStateInternalAsync`) returning ToolResults that surface the state-echo + benign reason; absent-service → explicit "Debugger service not available" error.
- `ServiceBootstrapper.cs`: wired `IDebuggerService` (lazy `GetService`) into the `ToolService` registration.
- Tests: extended `Fakes/FakeDebugger.cs` (`RunToCursorCalls` counter + flips mode; thread freeze/thaw already present). New `DebuggerInteropEvaluateMutateTests.cs` (interop: evaluate value/invalid, set-value local/argument/missing, memory benign not-exposed-by-dte, freeze/thaw flips `IsFrozen`, run-to-cursor counts+flips mode). New `ToolServiceDebugToolsTests.cs` (debug tools absent from `GetAvailableTools` in every mode by default; Debug-only + disabled via `GetTool`; route to fake `IDebuggerService` on invoke; rejected → state-echo reason; thaw→`ThawThreadAsync`; null service → unavailable). Updated registry/disabled counts in `BuiltInToolsTests`/`BuiltInToolsEnhancementTests` (28→34, disabled 7→13) and available-tool comments in `ToolServiceTests` (still 21 — debug tools default-disabled). `WorkspaceStatsServiceTests.StubDebuggerService` implements the 7 new interface members.
- Validation: `dotnet clean` → `dotnet build ContinueVS.slnx --force` (0 warnings, 0 errors) → `dotnet test`: **1607 passed, 0 failed, 0 skipped** (up from 1582).

**Deferred:** gap93 (routing), gap94 (lifecycle), gap95 (non-debug DTE). gap92 family (1/2/3) now complete.
...
**Objective:** Make the core contract real and add the state-precondition machine.

**Why it matters (the decisions that must hold):**
1. **`IDteProvider.GetDebugger()`** returns `EnvDTE.Debugger`, marshaled to the UI thread via the existing `ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync()` + `GetServiceAsync(typeof(DTE))` pattern; every `Debugger` call runs on the UI thread.
2. **`IsDebuggerActiveAsync`** reads `Debugger.CurrentMode` (`dbgRunMode`/`dbgBreakMode`) and/or `CurrentProgram != null` — real truth.
3. **`GetCurrentStateAsync`** walks the current session (`CurrentStackFrame` → locals/expressions, `StackFrames` → callstack, frame line/file) and still returns the benign `not-debugging` shape when not debugging. **This is the state echo** and must also return `{ mode, selectedThread, selectedFrame, breakReason }`.
4. **`SetBreakpointAsync`/`ClearBreakpointAsync`** add/delete from `Debugger.Breakpoints` by file+line with optional condition (incl. hit-count), matching by file/line on clear — not a fabricated GUID.
5. **`ExecuteStepAsync`** maps `DebugStepAction` → `StepOver()/StepInto()/StepOut()` then re-reads state; **`ResumeExecutionAsync`** calls `Debugger.Go(false)` with the existing 30s `ITimeoutHelper` guard.
6. **`DebugStateGuard`** is introduced here: every method validates state before invoking DTE and returns a benign rejection (not an exception). This is the enlarged-gap behavior that did not exist before.
7. Fail-soft: `COMException` from `Expression.Value`/`Type`/frames → benign state, so `WorkspaceStatsService.CollectDebugState` degrades to `DebugMode="none"`.

**Decided behavior:** `RuntimeState` shape preserved; add a `DebugSessionState` echo (mode/thread/frame/breakReason); `DebugStateGuard.Require(State)` gates every call.

**Service work required:**
- `IDteProvider`/`DteProvider` — expose `Debugger` (UI thread).
- `DebuggerService` — implement the five core methods + `DebugStateGuard` + state echo + `COMException` wrapping.

**Files (candidates):**
- `Services/Interfaces/IDteProvider.cs`, `Services/Implementations/DteProvider.cs`
- `Services/Interfaces/IDebuggerService.cs`, `Services/Implementations/DebuggerService.cs`
- `Core/Types/DebugSessionState.cs` (new), `Core/Types/DebugStateGuard.cs` (new)

**Testing strategy:** Fake-`Debugger` seam (no live VS): state reflects fake `CurrentMode`/frames/expressions; breakpoint add/clear round-trips against a fake `Breakpoints`; step maps enum → `StepInto/Over/Out`; resume calls `Go`; **out-of-state calls return the benign `{ok:false,reason}` shape, not an exception**; `COMException` → benign state. Replace `StubDebuggerService` in existing tests with this fake.

**Risks & decisions:** UI-thread affinity for all `EnvDTE`; cap locals/callstack depth so `CollectDebugState` stays responsive; keep `RuntimeState` stable. The `DebugStateGuard` is the code that makes the whole tool surface usable, so it lives here, not in `ToolService`.

**Sequencing:** First in the family. Depends on nothing in 92; prerequisite for 92_2/92_3/gap94.
...
**Objective:** Make the routing boundary explicit and single-source-of-truth: `ToolService` resolves `IDebuggerService` for `debug_*`; `IIdeService`/`VsIdeService` drops its debug stubs and keeps only non-debug DTE (covered by gap95). No new behavior — pure composition.

**Why it matters (the decisions that must hold):**
1. **Routing lives in `ToolService.InvokeBuiltInAsync`:** a `debug_*` name dispatches to `IDebuggerService` (injected), not to `IIdeService`.
2. **`VsIdeService` loses its debug methods** — they are deleted, not forwarded. This is the decisive change vs. the old gap93 (which forwarded via `IIdeService`).
3. **`IIdeService` keeps only non-debug DTE** (`ide_*` surface, gap95). Its `InspectVariablesAsync` debug surrogate is removed; any residual stat-context needs compose from `IDebuggerService` directly.
4. Keep null-service tolerance (benign no-op / `not-debugging` shape) consistent with `VsIdeService`'s optional-injection pattern.
5. No circular dependency: `ToolService` → `IDebuggerService` and `ToolService` → `IIdeService`; neither service depends on the other.
6. `DebugStepAction` mapping lives in exactly one place (`IDebuggerService`), per gap92_1.

**Decided behavior:** One facade (`IIdeService`, non-debug) + one debug automation service (`IDebuggerService`), both resolved by `ToolService`. No parallel debug logic anywhere.

**Service work required:**
- `ToolService` — add `IDebuggerService` ctor param; route `debug_*` (including state/guard/tier checks wired to gap92) directly to it.
- `VsIdeService` — remove the debug stub methods; keep only non-debug DTE (gap95).

**Files (candidates):**
- `Services/Implementations/ToolService.cs`
- `Services/Implementations/VsIdeService.cs`
- `Services/ServicesBootstrapper.cs` (registration/order check)

**Testing strategy:** `ToolService` tests assert `debug_*` calls hit the fake `IDebuggerService` and `ide_*`/file/git calls hit the fake `IIdeService`; `VsIdeService` tests assert no debug methods remain and non-debug DTE still works; null-service returns benign shapes. Existing `CollectDebugState` tests now compose from `IDebuggerService`, not a double-stub.

**Risks & decisions:** Avoid `IIdeService` ↔ `IDebuggerService` circular deps; keep the `debug_*` name → `IDebuggerService` routing table in one place; smallest gap.

**Sequencing:** Depends on gap92 (real `IDebuggerService`). Independent of gap94/gap95. Can be built and merged after 92_1.
...
**Problem (revised — it's a lifecycle gap, not a single Start):** The original gap94 was one method, `StartDebuggingAsync`, with attach & restart explicitly out of scope. The tool surface makes clear that's wrong: debug is entered by **Start** (startup project / launch profile) **or Attach** (to a running process), exited by **Stop**, and re-entered by **Restart** — and every later `debug_*` call must bind to a *selected* live session. So the "out of scope" notes for attach are rescinded; this gap owns the full lifecycle + the session-handle model.

**Objective:** Make the LLM go `not-debugging → debugging → stopped` and back, with an unambiguous "selected session" that all `debug_*` tools act on.

**Why it matters (the decisions that must hold):**
1. **Launch is distinct from resume.** `Debugger.Go`/`StepX` continue a session; starting one is `DTE.ExecuteCommand("Debug.Start")` on the startup project (or attach). Separate methods, not folded into `ResumeExecutionAsync`.
2. **New lifecycle methods on `IDebuggerService`:** `StartDebuggingAsync(project, launchProfile)` (with null = startup project), `AttachToProcessAsync(pid)`, `RestartDebuggingAsync()`, `StopDebuggingAsync()`. No longer a single `StartDebuggingAsync(project, launchProfile)` shape.
3. **Session-handle model lands here:** `GetSessionsAsync` / `SelectSessionAsync`; the rule *"every `debug_*` call binds to the currently selected session"* is enforced. This is the timeline/control-flow bridge.
4. **Startup/launch-profile resolution becomes real DTE plumbing** — reading `SolutionBuild.StartupProjects` and selecting a launch profile, not a `null` default hand-wave.
5. **Keep `ITimeoutHelper` + UI-thread marshaling** and fail-soft: no startup project / failed launch / attach failure → report reason, stay `not-debugging`, don't throw.
6. `Debug.Start` may prompt on first launch — surface a clear reason string; don't block the UI thread during launch (timeout + `SwitchToMainThreadAsync` only for the command issue).

**Decided behavior:** `IDebuggerService` exposes Start/Attach/Restart/Stop + session list/select. `ToolService` routes `debug_start`, `debug_stop`, `debug_restart`, `ide_attach_to_process` here. No UI surface in this gap — callable via tools.

**Service work required:**
- `IDebuggerService`/`DebuggerService` — lifecycle methods using `Debugger` accessor (gap92_1) + `IIdeService` startup-project resolution (gap95 `ide_launch_profile` can feed it).
- `ToolService` — wire the four lifecycle tool names to these methods + session selection.

**Files (candidates):**
- `Services/Interfaces/IDebuggerService.cs`, `Services/Implementations/DebuggerService.cs`
- `Services/Implementations/ToolService.cs`
- `Core/Types/DebugSessionInfo.cs` (new — session handles)

**Testing strategy:** Fake `Debugger` seam: `StartDebuggingAsync` issues `Debug.Start` and reports active when `CurrentMode` flips to run; attach issues `AttachToProcess` for a pid; restart/stop behave; no-startup-project fail-soft returns `false + reason`; `SelectSessionAsync` sets the binding used by later `debug_*` calls. End-to-end manual: LLM starts, then gap92 step/state/break work against the real launched process.

**Risks & decisions:** Launch prompts/timeouts; session selection must be explicit so out-of-band `debug_*` calls don't act on the wrong process; this is what makes {92, 93} reachable for a fresh app.

**Sequencing:** Depends on gap92_1 (accessor). Composes with gap93; uses gap95's launch-profile read. Required before the debug feature is user-visible end-to-end.
...
**Objective:** Add the non-debug `ide_*` DTE tool surface on `IIdeService`/`VsIdeService`, so the debugging workflow has its IDE context. All are DTE/`EnvDTE.SolutionBuild`/`OutputWindow`/`ItemOperations` operations on the UI thread, with the same fail-soft pattern as `VsIdeService`'s file ops.

**Why it matters (the decisions that must hold):**
1. **Owned by `IIdeService`, not `IDebuggerService`** — this is the gap93 Option A boundary made concrete.
2. Tools (Tier 1 unless noted):
   - `ide_active_document` — `DTE.ActiveDocument` (current file + selection) (Tier 0).
   - `ide_open_file` — `ItemOperations.OpenFile` / `Document.Activate`.
   - `ide_navigate_to` — `TextSelection.GotoLine` / `Edit.GoTo` (map breakpoints/statements visually).
   - `ide_goto_definition` — `Edit.GoToDefinition` (resolve what a breakpoint should target).
   - `ide_find_symbol` — resolve a symbol to file:line for breakpoints (feeds `debug_breakpoint_set`).
   - `ide_build` — `SolutionBuild.Build()` / `BuildProject()` and `ide_build_configuration` — set/read `SolutionConfigurations` (a stale build defeats debugging; this belongs in the loop).
   - `ide_launch_profile` — read/select `StartupProjects` + launch profile (feeds gap94 Start).
   - `ide_output_pane` — read any `OutputWindowPanes[...]` (Build/Debug) (Tier 0) — companion to `debug_output`.
   - `ide_command` — generic `DTE.ExecuteCommand(...)` passthrough for anything unwrapped (**Tier 2 → `ask`/disabled by default**, same gate story as gap92_3).
3. `ide_attach_to_process` — list local processes + `AttachToProcess` — is the attach entry; implemented here (it reads IDE process lists) but the actual attach lifecycle lands in gap94 (it's a debug-session action). Keep the boundary note in both gaps.
4. All run on the UI thread via the existing `SwitchToMainThreadAsync` pattern; `COMException` → benign error, never crash the session.
5. Register factories in `BuiltInToolsRegistry` + `ToolNameToUserSettingKey` with the same tier/user-settings gating as the rest.

**Decided behavior:** `IIdeService` grows the non-debug DTE surface; `debug_*` still routes to `IDebuggerService` (gap93). No debugger-automation code lives here.

**Service work required:**
- `VsIdeService` — implement the `ide_*` methods against `EnvDTE` (`SolutionBuild`, `OutputWindow`, `ItemOperations`, `TextSelection`, `ActiveDocument`), UI-thread + fail-soft.
- `ToolService` — register + route the `ide_*` names (Tier 1; `ide_command` Tier 2).
- `BuiltInToolsRegistry` + `ToolNameToUserSettingKey` — new factories + settings defaults.

**Files (candidates):**
- `Services/Implementations/VsIdeService.cs`
- `Services/Implementations/ToolService.cs`
- `Core/Types/BuiltInTools.cs`
- `Services/UserSettings.cs` (Tool_Ide*Enabled constants)

**Testing strategy:** Mock `DTE` seams (`SolutionBuild`, `OutputWindow`, `ItemOperations`, `TextSelection`): assert build issues `Build()`, launch-profile reads `StartupProjects`, output reads the right pane, command passthrough issues `ExecuteCommand`, navigate moves the selection; `COMException` → benign error; `ide_command` default gated to `ask`/disabled in the registry.

**Risks & decisions:** Keep `ide_*` strictly non-debugger to honor the Option A boundary; `ide_command` is a Tier-2 escape hatch — default-disabled and `ask`-gated; launch-profile and attach can be referenced from gap94 but are implemented here.

**Sequencing:** Independent of gap92/gap94 (pure `IIdeService` additions); composes with gap93 (same service) and feeds gap94. Can be built in parallel with the gap92 family.
...
|---|---|---|---|
| **gap92_1** | Real `Debugger` access + state guard + state echo + core break/step/resume | — | Tier 0/1 |
| **gap92_2** | Inspection: stack/frames/vars/threads/modules/exceptions/output | 92_1 | Tier 0 read |
| **gap92_3** | Evaluate/mutate: eval, set, memory, run-to-cursor, thread-state | 92_1, 92_2 | **Tier 2, gated** |
| **gap93** | Routing: `debug_*` → `IDebuggerService`, `IIdeService` = non-debug only | 92 family | — |
| **gap94** | Lifecycle: Start/Attach/Restart/Stop + session select | 92_1, 95 (profiles) | Tier 1 |
