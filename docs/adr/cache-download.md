# ADR: npm Cache Download on First Use

**Decision ID**: ADR-11  
**Title**: Cache-First Download Strategy for Continue npm Packages  
**Status**: Accepted  
**Date**: 2024-01-15  
**Related Steps**: Integrity validation, startup validation, npm installer  
...
1. **Avoid unnecessary downloads** — Check local cache first using validation
...
4. **Support offline use** — Allow pre-populated caches in air-gapped environments
5. **Be maintainable** — Use only Node.js built-ins, no external npm dependencies

**Technical constraints**:
- Integrity validation provides validation API
- Startup validation requires packages available before startup
- npm installer will use this module to fetch official v2.0.0 package
...
1. **Check local cache** — Call validation on cached package
...
4. **Validate after download** — Re-run validation to confirm integrity
...
| **Offline Fallback** | ✅ Via cache validation | ✗ Single point of failure | ✓ Included |
...
**Winner**: Cache-first provides best user experience for typical workflows while supporting offline via pre-population.
...
|-------|----------|----------|
| **Network timeout** | Log, retry once, collect error | Manual retry or offline fallback |
| **404 / 5xx HTTP** | Log, fail immediately | Check version, manual fix |
| **Empty download** | Delete temp file, collect error | Retry or offline fallback |
...
- No support for user version selection

**Rejected**: Too inflexible; cache-first + validation provides better solution.
...
| **Network dependency** | Offline fallback available | Cache-first + validation supports air-gapped via pre-population |
...
✅ **Offline support** — Validation can validate pre-cached packages without registry  
...
|------|------------|--------|-----------|
| **npm registry downtime** | Low | Bridge launch fails | Offline fallback (offline mode) |
...
| **User cache directory read-only** | Low | Clear error message | Directory permission check |
...
- Integration: End-to-end with real npm registry

**Dependencies**:
- ✅ Integrity validation (validates packages)
- ⏳ Startup validation (uses this module to ensure package available)
- ⏳ npm installer (uses this module to fetch official v2.0.0)
...
- **Integrity Validation**: `docs/npm-integrity-utility.md` — Validation API
- **Startup Validation**: `docs/adr/startup-validation.md` — Uses this module
- **npm Installer**: Download & verify official package
- **Strategy**: `docs/npm-cache-strategy.md` — Cache architecture

---

**Decision**: ✅ **Accepted** by Architecture Team  
**Implementation**: Cache download module  
